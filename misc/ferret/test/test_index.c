...
{
    Document **docs = FRT_ALLOC_N(Document *, BOOK_LIST_LENGTH);
    docs[0] = doc_new();
...
Document **prep_ir_test_docs()
{
    int i;
    char buf[2000] = "";
    Document **docs = FRT_ALLOC_N(Document *, IR_TEST_DOC_CNT);
    DocField *df;
...
static IndexReader *reader_test_env_ir_open(ReaderTestEnvironment *rte)
{
    if (rte->store_cnt == 1) {
        ...
        IndexReader **sub_readers = FRT_ALLOC_N(IndexReader *, rte->store_cnt);
        int i;
        for (i = 0; i < rte->store_cnt; i++) {
...
static ReaderTestEnvironment *reader_test_env_new(int type)
{
    ...
    rte->stores = FRT_ALLOC_N(Store *, store_cnt);
    ...
    if (type == add_indexes_reader_type) {
        ...
        FieldInfos *fis = fis_new(STORE_YES, INDEX_YES,
                                  TERM_VECTOR_WITH_POSITIONS_OFFSETS);
        IndexReader **readers = FRT_ALLOC_N(IndexReader *, rte->store_cnt);
        int i;
        for (i = 0; i < rte->store_cnt; i++) {
