...
class IndexTest < Test::Unit::TestCase
  ...
  def test_fs_index
    ...
    assert_raise(Isomorfeus::Ferret::FileNotFoundError) do
      Index.new(:path => fs_path, :create_if_missing => false, :default_field => :xxx)
    end
  ...
  def test_doc_specific_analyzer
    index = Index.new
    index.add_document("abc", Isomorfeus::Ferret::Analysis::Analyzer.new)
    assert_equal(1, index.size)
  end
  ...
  def test_highlighter
    index = Isomorfeus::Ferret::I.new(:default_field => :field,
  ...
  def test_changing_analyzer
    index = Isomorfeus::Ferret::I.new
    a = Isomorfeus::Ferret::Analysis::WhiteSpaceAnalyzer.new(false)
  ...
  def test_top_doc_to_json
    ...
    json_str = index.search("one two 3 4.0 five",
                            :sort => Isomorfeus::Ferret::Search::Sort::INDEX_ORDER).to_json
