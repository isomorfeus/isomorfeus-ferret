...
class QueryParserTest < Test::Unit::TestCase
  ...
  def test_qp_changing_fields
    ...
    assert_equal('word one:word two:word three:word', parser.parse("*:word").to_s("xxx"))
  ...
  def test_qp_allow_any_field
    ...
    assert_equal('key:word', parser.parse("key:word song:word").to_s("xxx"))
    assert_equal('word key:word', parser.parse("*:word").to_s("xxx"))
  ...
  def do_test_query_parse_exception_raised(str)
    ...
    assert_raise(Isomorfeus::Ferret::QueryParser::QueryParseException,
  ...
  def test_or_default
    parser = Isomorfeus::Ferret::QueryParser.new(:default_field => :*,
                                     :fields => [:x, :y],
                                     :or_default => false,
                                     :analyzer => StandardAnalyzer.new)
    pairs = [
  ...
  def test_prefix_query
    parser = Isomorfeus::Ferret::QueryParser.new(:default_field => "xxx",
                                     :fields => ["xxx"],
                                     :analyzer => StandardAnalyzer.new)
    assert_equal(Isomorfeus::Ferret::Search::PrefixQuery, parser.parse("asdg*").class)
  ...
  def test_use_keywords_switch
    ...
    parser = Isomorfeus::Ferret::QueryParser.new(:analyzer => analyzer,
                                     :default_field => "xxx")
    assert_equal("+www (+xxx +yyy) -zzz", parser.parse("REQ www (xxx AND yyy) OR NOT zzz").to_s("xxx"))

    parser = Isomorfeus::Ferret::QueryParser.new(:analyzer => analyzer,
                                     :default_field => "xxx",
                                     :use_keywords => false)
    assert_equal("req www (xxx and yyy) or not zzz", parser.parse("REQ www (xxx AND yyy) OR NOT zzz").to_s("xxx"))
  end
end
