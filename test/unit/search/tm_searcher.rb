...
module SearcherTests
  ...
  def test_term_query
    ...
    assert_equal([Isomorfeus::Ferret::Term.new(:field, "word1")], tq.terms(@searcher))
  end
  ...
  def test_multi_term_query
    ...
    assert(terms.index(Isomorfeus::Ferret::Term.new(:field, "word2")))
    assert(terms.index(Isomorfeus::Ferret::Term.new(:field, "fast")))
  end
