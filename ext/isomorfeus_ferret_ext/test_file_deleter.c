...
{
    create_index(store);
    return frt_iw_open(store, frt_whitespace_analyzer_new(false, NULL), &lucene_config);
}
...
static void test_delete_leftover_files(TestCase *tc, void *data)
{
    ...
    frt_iw_close(frt_iw_open(store, frt_whitespace_analyzer_new(false, NULL), &lucene_config));
