...
static FrtIndexWriter *create_book_iw_conf(FrtStore *store, const FrtConfig *config)
{
    ...
    frt_fis_deref(fis);
    return frt_iw_open(store, frt_whitespace_analyzer_new(false, NULL), config);
}
...
static void test_index_version(TestCase *tc, void *data)
{
    ...
    iw = frt_iw_open(store, frt_whitespace_analyzer_new(false, NULL), &frt_default_config);
...
static void test_simulated_crashed_writer(TestCase *tc, void *data)
{
    ...
    iw = frt_iw_open(store, frt_whitespace_analyzer_new(false, NULL), &config);
...
static void test_ir_norms(TestCase *tc, void *data)
{
    ...
    for (i = 0; i < rte->store_cnt; i++) {
        iw = frt_iw_open(rte->stores[i], frt_whitespace_analyzer_new(false, NULL),
                     &frt_default_config);
        frt_iw_optimize(iw);
        frt_iw_close(iw);
    }
...
static void test_ir_read_while_optimizing(TestCase *tc, void *data)
{
    ...
    iw = frt_iw_open(store, frt_whitespace_analyzer_new(false, NULL), false);
    frt_iw_optimize(iw);
    frt_iw_close(iw);
