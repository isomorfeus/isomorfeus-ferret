                                 ...
                                 int doc_cnt, FrtFieldInfos *fis)
{
    ...
    frt_index_create(store, fis);
    iw = frt_iw_open(store, frt_mb_whitespace_analyzer_new(false), NULL);
...
static void test_index_version(TestCase *tc, void *data)
{
    ...
    iw = frt_iw_open(store, frt_mb_whitespace_analyzer_new(false), &frt_default_config);
...
static void test_simulated_crashed_writer(TestCase *tc, void *data)
{
    ...
    iw = frt_iw_open(store, frt_mb_whitespace_analyzer_new(false), &config);
...
static void test_ir_norms(TestCase *tc, void *data)
{
    ...
    for (i = 0; i < rte->store_cnt; i++) {
        iw = frt_iw_open(rte->stores[i], frt_mb_whitespace_analyzer_new(false),
                     &frt_default_config);
        frt_iw_optimize(iw);
        frt_iw_close(iw);
    }
...
static void test_ir_read_while_optimizing(TestCase *tc, void *data)
{
    ...
    iw = frt_iw_open(store, frt_mb_whitespace_analyzer_new(false), false);
    frt_iw_optimize(iw);
    frt_iw_close(iw);
