...
{
    FrtToken *tk = frt_tk_new();
    FrtTokenStream *ts = frt_whitespace_tokenizer_new(false, NULL);
    char text[100] = "DBalmain@gmail.com is My e-mail 52   #$ address. 23#!$";
    (void)data;
    rb_encoding *enc = rb_enc_find("ASCII-8BIT");
    ts->reset(ts, text, enc);
...
static void test_mb_whitespace_tokenizer(TestCase *tc, void *data)
{
    ...
    Assert(frt_ts_next(ts) == NULL, "Should be no more tokens");
    frt_ts_deref(ts);
    ts = frt_whitespace_tokenizer_new(true, NULL);
    ts->reset(ts, text, enc);
    test_token(frt_ts_next(ts), "dbalmän@gmail.com", 0, 18, enc);
    test_token(frt_ts_next(ts), "is", 19, 21, enc);
    test_token(frt_ts_next(ts), "my", 22, 24, enc);
...
{
    FrtToken *tk = frt_tk_new();
    FrtAnalyzer *a = frt_whitespace_analyzer_new(false, NULL);
    char text[100] = "DBalmain@gmail.com is My e-mail 52   #$ address. 23#!$";
    rb_encoding *enc = rb_enc_find("ASCII-8BIT");
    FrtTokenStream *ts = frt_a_get_ts(a, rb_intern("random"), text, enc);
    (void)data;
...
static void test_mb_whitespace_analyzer(TestCase *tc, void *data)
{
    ...
    frt_ts_deref(ts);
    frt_a_deref(a);
    a = frt_whitespace_analyzer_new(true, NULL);
...
{
    FrtToken *tk = frt_tk_new();
    FrtTokenStream *ts = frt_letter_tokenizer_new(false, NULL);
    char text[100] = "DBalmain@gmail.com is My e-mail 52   #$ address. 23#!$";
    (void)data;
    rb_encoding *enc = rb_enc_find("ASCII-8BIT");
    ts->reset(ts, text, enc);
...
static void test_mb_letter_tokenizer(TestCase *tc, void *data)
{
    ...
    Assert(frt_ts_next(ts) == NULL, "Should be no more tokens");
    frt_ts_deref(ts);
    ts = frt_letter_tokenizer_new(true, NULL);
    ts->reset(ts, text, enc);
    test_token(frt_ts_next(ts), "dbalmän", 0, 8, enc);
    test_token(frt_ts_next(ts), "gmail", 9, 14, enc);
    test_token(frt_ts_next(ts), "com", 15, 18, enc);
...
{
    FrtToken *tk = frt_tk_new();
    FrtAnalyzer *a = frt_letter_analyzer_new(false, NULL);
    char text[100] =
    ...
    frt_ts_deref(ts);
    frt_a_deref(a);
    a = frt_letter_analyzer_new(true, NULL);
    ts = frt_a_get_ts(a, rb_intern("random"), text, enc);
    test_token(frt_ts_next(ts), "dbalmän", 0, 8, enc);
    test_token(frt_ts_next(ts), "gmail", 9, 14, enc);
    test_token(frt_ts_next(ts), "com", 15, 18, enc);
...
{
    FrtTokenStream *ts = frt_standard_tokenizer_new(false, NULL);
    (void)data;
...
{
    FrtToken *tk = frt_tk_new();
    FrtAnalyzer *a = frt_standard_analyzer_new_with_words(FRT_ENGLISH_STOP_WORDS, true, NULL);
    char text[200] =
...
static void test_mb_standard_analyzer(TestCase *tc, void *data)
{
    FrtToken *tk = frt_tk_new();
    FrtAnalyzer *a =
        frt_standard_analyzer_new_with_words(FRT_ENGLISH_STOP_WORDS, false, NULL);
    ...
    frt_ts_deref(ts);
    frt_a_deref(a);
    a = frt_standard_analyzer_new(true, NULL);
    ts = frt_a_get_ts(a, rb_intern("random"), text, enc);
    test_token_pi(frt_ts_next(ts), "dbalmain@gmail.com", 0, 18, 1, enc);
    ...
    frt_ts_deref(ts);
    frt_a_deref(a);
    a = frt_standard_analyzer_new_with_words(words, true, NULL);
    ts = frt_a_get_ts(a, rb_intern("random"), text, enc);
...
static void test_legacy_standard_analyzer(TestCase *tc, void *data)
{
    FrtToken *tk = frt_tk_new();
    FrtAnalyzer *a =
        frt_standard_analyzer_new_with_words(FRT_ENGLISH_STOP_WORDS, true, NULL);
    char text[200] =
        "DBalmain@gmail.com is My e-mail and the Address. -23!$ "
        "http://www.google.com/results/ T.N.T. 123-1235-ASD-1234";
    rb_encoding *enc = rb_enc_find("ASCII-8BIT");
...
static void test_long_word(TestCase *tc, void *data)
{
    ...
    frt_ts_deref(ts);
    frt_a_deref(a);
    a = frt_standard_analyzer_new_with_words(FRT_ENGLISH_STOP_WORDS, true, NULL);
    ts = frt_a_get_ts(a, rb_intern("random"), text, enc);
    test_token_pi(frt_ts_next(ts), text_a, 0, 290, 1, enc);
    test_token_pi(frt_ts_next(ts), "two", 291, 294, 1, enc);
    Assert(frt_ts_next(ts) == NULL, "Should be no more tokens");
...
{
    FrtToken *tk = frt_tk_new();
    FrtTokenStream *ts = frt_standard_tokenizer_new(true, NULL);
    char text[200] =
...
{
    FrtToken *tk = frt_tk_new();
    FrtTokenStream *ts = frt_hyphen_filter_new(frt_standard_tokenizer_new(true, NULL));
    char text[200] =
...
static void test_stop_filter(TestCase *tc, void *data)
{
    FrtToken *tk = frt_tk_new();
    FrtTokenStream *ts =
        frt_stop_filter_new_with_words(frt_letter_tokenizer_new(false, NULL), words);
    char text[200] =
...
{
    FrtToken *tk = frt_tk_new();
    FrtTokenStream *ts = frt_mapping_filter_new(frt_letter_tokenizer_new(false, NULL));
    char text[200] =
        "one, two, three, four, five, six, seven, eight, nine, ten.";
    char long_word[301] =
...
{
    FrtToken *tk = frt_tk_new();
    FrtTokenStream *ts = frt_stem_filter_new(frt_letter_tokenizer_new(true, NULL), "english");
    FrtTokenStream *ts2;
...
static void test_per_field_analyzer(TestCase *tc, void *data)
{
    ...
    FrtToken *tk = frt_tk_new();
    char text[100] = "DBalmain@gmail.com is My E-mail 52   #$ address. 23#!$";
    FrtAnalyzer *pfa = frt_per_field_analyzer_new(frt_standard_analyzer_new(true, NULL));
    (void)data;
    rb_encoding *enc = utf8_encoding;

    frt_pfa_add_field(pfa, rb_intern("white"), frt_whitespace_analyzer_new(false, NULL));
    frt_pfa_add_field(pfa, rb_intern("white_l"), frt_whitespace_analyzer_new(true, NULL));
    frt_pfa_add_field(pfa, rb_intern("letter"), frt_letter_analyzer_new(false, NULL));
    frt_pfa_add_field(pfa, rb_intern("letter"), frt_letter_analyzer_new(true, NULL));
    frt_pfa_add_field(pfa, rb_intern("letter_u"), frt_letter_analyzer_new(false, NULL));
